...
pub fn ensure_environment() -> Result<(), String> {
    ...
    println!("[defender-rs][env] wscsvc is not running, starting it...");
    if !start_service(svc) {
        return Err("Failed to start wscsvc service".to_string());
    }

    println!("[defender-rs][env] successfully started wscsvc, waiting for it to get up...");
    for _ in 0..20 {
        thread::sleep(Duration::from_millis(500));
        if query_service_running(svc) {
            println!("[defender-rs][env] wscsvc is running, environment ready");
            return Ok(());
        }
    }
